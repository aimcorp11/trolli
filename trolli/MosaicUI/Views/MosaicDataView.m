...
#define kMosaicDataViewFont @"Helvetica-Regular"
...
//    NSLog(@"#DEBUG imageRect %.2f %.2f (%.2f %.2f) %@", imgFinalSize.width, imgFinalSize.height, anImage.size.width, anImage.size.height, newModule);
    ...
    imageView.frame = CGRectMake(5, 5, imgFinalSize.width-10, imgFinalSize.height-10);
    ...
    if ([[[UIDevice currentDevice] systemVersion] compare:@"7.0" options:NSNumericSearch] != NSOrderedAscending) {
        CGRect r = [module.title boundingRectWithSize:titleLabel.frame.size
                                                     options:NSStringDrawingUsesLineFragmentOrigin
                                                  attributes:@{NSFontAttributeName:titleLabel.font}
                                                     context:nil];
        newSize = CGSizeMake(r.size.width, r.size.height);
    } else {
